...
original_dir = path.join(path.dirname(__file__), "./dataset/original3")
...
corpora_csv_core10 = path.join(processed_dir, "corpora-core10.csv")
posts_json = path.join(processed_dir, "posts.json")
posts_json_core10 = path.join(processed_dir, "posts-core10.json")

stoplist_csv = path.join(processed_dir, "stoplist.csv")

vocab_json = path.join(processed_dir, "vocab.json")
vocab_json_core10 = path.join(processed_dir, "vocab-core10.json")
tags_json = path.join(processed_dir, "tags.json")
vocab_json_core50 = path.join(processed_dir, "tags-core50.json")
...
def download():
    ...
    cursor.execute(
        ...
        WHERE score >= 5 AND Tags IS NOT NULL")
    r = cursor.fetchall()
...
def filterData():
    ...
    for word, count in vocab_dict.items():
        ...
        #     vocab[word] = count
        if count < 10 or (pattern1.match(word) is not None or pattern2.search(word) is not None or pattern3.search(word) is not None):
            continue
        vocab[word] = count
    ...
    with open(posts_json_core10, "w", newline="", encoding='UTF-8') as json_file:
        json.dump(dataset, json_file)
    with open(vocab_json_core50, "w", newline="", encoding='UTF-8') as json_file:
        json.dump(tag_vocab, json_file)
    with open(vocab_json_core10, "w", newline="", encoding='UTF-8') as json_file:
        json.dump(vocab, json_file)   
    # with open(corpora_csv_core10, "w", newline="", encoding='UTF-8') as csv_file:
    #     writer = csv.writer(csv_file, dialect='excel')
    #     writer.writerows(sentences)
...
def gen_corpora(valid_portion=0.1):
    ...
    with open(posts_json_core10, "r", encoding='UTF-8') as json_file:
...
def load_vocab():
    vocab = []
    vocab_index = {}
    with open(vocab_json_core10, "r", encoding='UTF-8') as json_file:
        vocab_dict = json.load(json_file)
...
def load_tag_vocab():
    tag_vocab = []
    tag_index = {}
    with open(vocab_json_core50, "r", encoding='UTF-8') as json_file:
        tag_vocab_dict = json.load(json_file)
