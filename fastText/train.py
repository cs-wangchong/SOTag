...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        # Instantiate Model
        fastText = FastText(FLAGS.k, vocab_size, tag_vocab_size, FLAGS.learning_rate, FLAGS.num_epochs,
        ...
        if path.exists(path.join(ckpt_dir, "checkpoint")):
            ...
            if FLAGS.use_embedding:  # load pre-trained word embedding
                assign_pretrained_word_embedding(sess, vocab, vocab_size, fastText, word2vec_model_path=FLAGS.word2vec_model_path)
        curr_epoch = sess.run(fastText.epoch_step)
        # 3.feed data & training
        number_of_training_data = len(trainX)
        batch_size = FLAGS.batch_size
        # num_batchs = int(number_of_training_data / batch_size) + 1
        print("Training...")
        print("Start time: {}".format(time.asctime(time.localtime(time.time()))))
        for epoch in range(curr_epoch, FLAGS.num_epochs):
            print("Epoch: %d\tlr: %.3f" % (epoch, fastText.lr))
            loss, recall, precision, counter = 0.0, 0.0, 0.0, 0
            for start, end in zip(range(0, number_of_training_data, batch_size), range(batch_size, number_of_training_data, batch_size)):

                feed_dict = {fastText.input_x: trainX[start:end], fastText.input_length: train_sent_len[start:end], fastText.dropout_keep_prob: 0.5}
                input_y = [labels_2_onehot(y, tag_vocab_size) for y in trainY[start:end]]
                feed_dict[fastText.input_y] = input_y
                curr_loss, topK, _ = sess.run([fastText.loss_val, fastText.topK, fastText.train_op], feed_dict)  # curr_f1--->fastText.f1
                ...
                tp_op, fp_op, fn_op = sess.run([fastText.tp_op, fastText.fp_op, fastText.fn_op], feed_dict={
                                               fastText.pred: pred_nparr, fastText.actual: actual})
            ...
            if epoch % FLAGS.validate_every == 0:
                eval_loss, eval_recall, eval_precision, eval_f1 = do_eval(sess, fastText, validX, validY, valid_sent_len, batch_size, tag_vocab_size)
            ...
            _, lr = sess.run([fastText.epoch_increment, fastText.lr_decay])
            fastText.lr = lr
        print("End time: {}".format(time.asctime(time.localtime(time.time()))))
        # 5.最后在测试集上做测试，并报告测试准确率 Test
        test_loss, test_recall, test_precision, test_f1 = do_eval(sess, fastText, testX, testY, test_sent_len, batch_size, tag_vocab_size)
        print("Test Loss:%.3f\tRecall: %.3f\tPrecision: %.3f\tF1: %.3f" % (test_loss, test_recall, test_precision, test_f1))
    pass


def assign_pretrained_word_embedding(sess, vocab, vocab_size, fastText, word2vec_model_path=None):
    ...
    t_assign_embedding = tf.assign(fastText.Embedding, word_embedding)  # assign this value to our embedding variables of our model.
    sess.run(t_assign_embedding)
    print("Word exists embedding:", count_exist, "; Word not exist embedding:", count_not_exist)


def do_eval(sess, fastText, evalX, evalY, sent_len, batch_size, tag_vocab_size):
    number_examples = len(evalX)
    eval_loss, recall, precision, eval_counter = 0.0, 0.0, 0.0, 0
    for start, end in zip(range(0, number_examples, batch_size), range(batch_size, number_examples, batch_size)):
        feed_dict = {fastText.input_x: evalX[start:end], fastText.input_length: sent_len[start:end], fastText.dropout_keep_prob: 1}
        input_y = [labels_2_onehot(y, tag_vocab_size) for y in evalY[start:end]]
        feed_dict[fastText.input_y] = input_y
        curr_eval_loss, topK = sess.run([fastText.loss_val, fastText.topK], feed_dict)  # curr_f1--->fastText.f1
        pred_nparr = np.zeros([batch_size, tag_vocab_size])
        for i in range(batch_size):
            pred_nparr[i][topK[i]] = 1
