...
original_dir = path.join(path.dirname(__file__), "./dataset/original")
processed_dir = path.join(path.dirname(__file__), "./dataset/processed")
...
def download():
    ...
    cursor.execute(
        ...
        WHERE score > 100 AND Tags IS NOT NULL")
    # cursor.execute(
    #      "SELECT Id, Body, OwnerUserId, Title, Tags, CreationDate FROM posts LIMIT 0, 100")
    r = cursor.fetchall()

    headers = ["Id", "Body", "OwnerUserId", "Title", "Tags", "CreationDate"]
...
def preprocess():
    ...
    with open(posts_csv, "r", encoding='UTF-8') as csv_file:
        ...
        for row in reader:
            ...
            new_tags = [wnl.lemmatize(e.strip('<> ').lower()) for e in row[4].split('><')]
            processed_data.append([row[0], new_body, row[2], new_title, new_tags, row[5], code_list])
            for tag in new_tags:
...
def filterData():
    ...
    for data in dataset_raw:
        ...
        for word in data[3]:
            if vocab.get(word, None) is not None:
                title.append(word)
        dataset.append([data[0], body, data[2], title, tags, data[5], data[6]])
...
def load_dataset(vocab_index, tag_index, tag_vocab_size, valid_portion=0.1):
    ...
    # print("sentence average length: %f" % (word_size / dataset_size))
